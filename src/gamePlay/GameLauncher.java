...
//        this.pieces.add(new king(true,"F8",game));
    ...
    public void Clock(String clickedSquare) {
        gameStatus = ChessBoardBASE.gameResult;
    ...
    private void removePiece(String pos) {
        for(int i = 0; i < this.pieces.size(); ++i) {
            if (this.pieces.get(i).getPosition().equals(pos)) {
    ...
    public boolean ally(Piece mine, String pos) {
        if (this.getPiece(pos) == null) {
            ...
            return Objects.requireNonNull(this.getPiece(pos)).pieceSide == mine.pieceSide;
        }
    }
    public void start(){
        game.show();
